...
import blogData from "../assets/C_S_Data";
...
const C_S_Details = () => {
  ...
  }, [blog.title]); // Use slug instead of blog.title
      ...
      <div className="w-full bg-gray-100 mt-[90px] py-12 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-8 rounded-lg shadow-xl">
            <img src={blog.imgUrl} alt={blog.title} className="w-full rounded-lg shadow-md" />
            ...
            <section className="mt-8">
              ...
              <ul className="list-disc pl-6 mt-3 text-gray-700 space-y-2">
                ...
                <li>{blog.challenges2}</li>
                <li>{blog.challenges3}</li>
              </ul>
            </section>
            ...
            <div className="space-y-4">
              {[...blogData].reverse().map((item) => (
                <div key={item.id} className="flex gap-4 items-center border-b pb-4 last:border-b-0">
                  <img src={item.imgUrl} alt={item.title} className="w-16 h-16 object-cover rounded-lg shadow" />
                  <Link to={`/blogs/${item.title}`} className="text-blue-600 font-semibold hover:underline">
                    {item.title}
                  </Link>
                </div>
              ))}
